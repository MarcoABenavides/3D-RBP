...
def create_transformer_cnn_model(input_shape, num_classes=34, transformer_params=None):
    ...
    # Preserve sequence structure (don't Flatten)
...
if __name__ == "__main__":
    ...
    # Reshape the data to 2D for MinMaxScaler
    ...
    # Train and evaluate the model with EarlyStopping
# Train and evaluate the model with EarlyStopping
model.fit(
    X_train_scaled,
    y_train,
...
# Retrieve metrics from the callback
...
# Save predictions for test and validation sets
ground_truth_val_test = y_test
predicted_classes_val_test = predicted_classes

# Save to CSV
output_val_test_file = os.path.join(OUTPUT_DIR, "predictions_ground_truth-transformer-CNN+3d-data-softmax.csv")
pd.DataFrame({
    "Ground_Truth": ground_truth_val_test,
    "Predicted_Class": predicted_classes_val_test
}).to_csv(output_val_test_file, index=False)

print(f"[INFO] Ground truth and predictions saved to {output_val_test_file}")
...
# Save epoch metrics
results_file = os.path.join(OUTPUT_DIR, "balanced-CNN-transformer+3d-data-softmax.txt")
